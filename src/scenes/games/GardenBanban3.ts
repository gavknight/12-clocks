...
export class GardenBanban3 {
  ...
  private _setupTouchControls(): void {
    ...
    const medalBtn = document.createElement("div");
    medalBtn.textContent = "🏅";
    medalBtn.style.cssText =
      "position:absolute;bottom:170px;right:105px;width:60px;height:60px;" +
      "border-radius:50%;background:rgba(255,160,40,0.25);border:2px solid rgba(255,160,40,0.5);" +
      "display:flex;align-items:center;justify-content:center;font-size:26px;" +
      "pointer-events:all;touch-action:none;user-select:none;";
    hud.appendChild(medalBtn);
    medalBtn.addEventListener("touchstart", e => {
      e.preventDefault();
      this._keys.add("KeyF");
      if (!this._started) this._doStart();
    }, { passive: false });
    medalBtn.addEventListener("touchend", () => this._keys.delete("KeyF"));
  }
