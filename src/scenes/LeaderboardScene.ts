...
export class LeaderboardScene {
  constructor(game: Game) {
    ...
    game.getRecords().then(records => this._build(game, records));
  }

  private _showLoading(game: Game): void {
    game.ui.innerHTML = `
      <div class="screen" style="background:linear-gradient(160deg,#0a0020,#1a0850,#2a0060);
        flex-direction:column;gap:16px;">
        <div style="font-size:40px;">🏆</div>
        <div style="color:rgba(255,255,255,0.6);font-size:16px;">Loading records...</div>
      </div>`;
  }

  private _build(game: Game, records: Awaited<ReturnType<typeof game.getRecords>>): void {
    const myId = game.currentAccountId;

    const rows = records.length === 0
      ? `<div style="color:rgba(255,255,255,0.4);font-size:15px;text-align:center;padding:20px;">
           No Hard mode completions yet!<br>
           <span style="font-size:13px;">Finish all 12 puzzles on Hard to appear here.</span>
         </div>`
      : records.map((rec, i) => {
          const isMe  = rec.accountId === myId;
    ...
    game.ui.innerHTML = `
      <div class="screen" style="
        background:linear-gradient(160deg,#0a0020,#1a0850,#2a0060);
      ...
      ">
        ...
        <p style="color:rgba(255,255,255,0.45);font-size:13px;margin:0 0 20px;">
          Fastest Hard mode completions (all 12 puzzles)
        </p>
