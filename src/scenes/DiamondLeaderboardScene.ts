...
export class DiamondLeaderboardScene {
  constructor(game: Game) {
    game._disposeScene = () => { game.ui.innerHTML = ""; };
    this._showLoading(game);
    // Push current player's diamonds first, then fetch so they appear immediately
    game.syncDiamonds().then(() => game.getDiamondLeaderboard()).then(records => this._build(game, records));
  }

  private _showLoading(game: Game): void {
    game.ui.innerHTML = `
      <div class="screen" style="background:linear-gradient(160deg,#001020,#082040,#001830);
  ...
  private _build(game: Game, records: Awaited<ReturnType<typeof game.getDiamondLeaderboard>>): void {
    const myId = game.currentAccountId;

    const rows = records.length === 0
      ? `<div style="color:rgba(255,255,255,0.4);font-size:15px;text-align:center;padding:20px;">
           No diamonds gifted yet!<br>
           <span style="font-size:13px;">Diamonds are admin-gifted only.</span>
         </div>`
      : records.map((rec, i) => {
          ...
          return `
            <div style="
              ...
              border:${isMe
                ? "2px solid rgba(100,220,255,0.5)"
                : i === 0
                  ? "2px solid rgba(100,220,255,0.25)"
            ...
            ">
              ...
              <div style="
                ...
                color:${i === 0 ? "#66ddff" : "rgba(255,255,255,0.85)"};
                white-space:nowrap;
              ">
                💎 ${rec.diamonds.toLocaleString()}
              </div>
            </div>`;
        }).join("");
    ...
    game.ui.innerHTML = `
      <div class="screen" style="
        background:linear-gradient(160deg,#001020,#082040,#001830);
      ...
      ">
        ...
        <h2 style="color:#66ddff;font-size:30px;margin:0 0 4px;
          text-shadow:0 0 16px rgba(100,220,255,0.5);">
          Diamond Leaderboard
        </h2>
        <p style="color:rgba(255,255,255,0.45);font-size:13px;margin:0 0 20px;">
          Most diamonds gifted by admins
        </p>
